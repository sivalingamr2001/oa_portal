...
function FulfillmentLinesPanel({
    lines,
    columns,
    soLineColumns,
}: {
    lines: AllocationFulfillment[];
    columns: ColumnsType<AllocationFulfillment>;
    soLineColumns: ColumnsType<SalesOrderLine>;
}) {
...
export default function FulfillmentTracker({ currentUser }: { currentUser: string }) {
    ...
    const handleGroupExpand = (expanded: boolean, record: FulfillmentDateGroup) => {
        setExpandedGroupKeys(prev =>
            expanded
                ? [...prev, record.groupKey]
                : prev.filter(key => key !== record.groupKey)
        );
    };
    ...
    return (
        <>
            ...
            <DynamicGrid<FulfillmentDateGroup>
                columns={[
                    {
                        title: 'DATE',
                        key: 'date',
                        width: 220,
                        render: (_: unknown, record: FulfillmentDateGroup) => (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                                <span style={{ color: '#1d4ed8', fontWeight: 700, fontSize: '13px' }}>{record.displayDate}</span>
                                <span style={{ color: '#64748b', fontSize: '12px' }}>
                                    {record.lines.length} line{record.lines.length === 1 ? '' : 's'}
                                </span>
                            </div>
                        ),
                    },
                ...
                expandable={{
                    ...
                    expandedRowRender: record => (
                        <FulfillmentLinesPanel
                            lines={record.lines}
                            columns={mainColumns}
                            soLineColumns={soLineColumns}
                        />
                    ),
                }}
            />
